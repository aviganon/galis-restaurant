...
export async function extractWithAI(
  ...
  if (isRtf) {
    ...
    const userContent =
      type === "p"
        ? "נתח את החשבונית: חלץ שם ספק, תאריך, ולכל פריט — name=עמודת תאור/שם המוצר (לא המק"ט!), sku=מק\"ט, price=מחיר נטו (אחרי הנחה), unit=יחידה, qty=כמות. אם אין מחירים כלל — no_prices:true. JSON בלבד."
        : type === "d"
