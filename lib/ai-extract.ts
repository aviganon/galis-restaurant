...
${SUPPLIER_KNOWLEDGE}
חוקים: name=תיאור המוצר בלבד (עמודת "תאור מוצר" — חובה, לא ריק!). sku=מק"ט/ברקוד (קוד מספרי נפרד). מחיר נטו ליחידה. qty=כמות שהתקבלה.
שם ספק: חלץ תמיד — לרוב בראש המסמך (שם החברה, היצרן או הספק). דוגמאות: "הכרם - משקאות חריפים", "תנובה", "אסם".
לכל רכיב: name (תיאור מלא), price (מחיר נטו ליחידה), unit (יחידה), sku (מק"ט אם קיים), qty (כמות שהתקבלה).
...
export async function extractWithAI(
  ...
  if (isRtf) {
    ...
    const userContent =
      type === "p"
        ? "נתח את המסמך. לכל פריט: name=תיאור המוצר (עמודת תאור מוצר, חובה!), sku=מק\"ט המספרי, unit=יחידה, qty=כמות. אם יש מחירים — price=מחיר נטו. אם אין מחירים (תעודת משלוח) — no_prices:true ו-price:0. JSON בלבד."
        : type === "d"
