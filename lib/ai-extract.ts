...
function parseSupplierNumericField(v: unknown): number {
  if (typeof v === "number" && !Number.isNaN(v)) return v
  if (typeof v === "string") {
    const s = v.replace(/[\s₪NIS]/gi, "").replace(/,/g, ".").trim()
    if (!s) return NaN
...
/** מפתחות בעברית או שגויים בתשובת AI — ממופים לשדות ExtractedSupplierItem */
const SUPPLIER_NAME_KEY_CANDIDATES = [
  "name",
  "שם",
  "שם פריט",
  "שם מוצר",
...
const SUPPLIER_SKU_KEY_CANDIDATES = [
  "sku",
  "מקט",
  'מק"ט',
  "מק״ט",
...
const SUPPLIER_PRICE_KEY_CANDIDATES = [
  "price",
  "מחיר",
  "מחיר חשבונית",
  "מחיר ליחידה",
...
const SUPPLIER_QTY_KEY_CANDIDATES = ["qty", "quantity", "כמות", "כמויות"]

const SUPPLIER_UNIT_KEY_CANDIDATES = ["unit", "יחידה", "יח", "uom"]

function pickFirstStringField(row: Record<string, unknown>, candidates: string[]): string {
...
/** התאמת מפתחות דינמית (גרשיים שונים ב־JSON, רווחים) */
function normalizeExcelishKey(s: string): string {
  return s
    .trim()
    .toLowerCase()
    .replace(/[\s_\u00a0]+/g, "")
...
function findRowStringByNormalizedKeys(row: Record<string, unknown>, synonyms: string[]): string {
  const targets = new Set(synonyms.map(normalizeExcelishKey).filter(Boolean))
  for (const [k, v] of Object.entries(row)) {
    if (typeof v !== "string" || !v.trim()) continue
    if (targets.has(normalizeExcelishKey(k))) return v.trim()
  }
  return ""
}

function findRowNumberByNormalizedKeys(row: Record<string, unknown>, synonyms: string[]): number {
  const targets = new Set(synonyms.map(normalizeExcelishKey).filter(Boolean))
  for (const [k, v] of Object.entries(row)) {
    if (!targets.has(normalizeExcelishKey(k))) continue
    const n = parseSupplierNumericField(v)
    if (!Number.isNaN(n)) return n
  }
  return NaN
}

function pickFirstNumberField(row: Record<string, unknown>, candidates: string[]): number {
  for (const k of candidates) {
    if (!Object.prototype.hasOwnProperty.call(row, k)) continue
    const n = parseSupplierNumericField(row[k])
    if (!Number.isNaN(n)) return n
  }
  return NaN
}

function inferSupplierNameFromLooseKeys(row: Record<string, unknown>, consumed: Set<string>): string {
  const skip = new Set([
    ...SUPPLIER_SKU_KEY_CANDIDATES,
    ...SUPPLIER_PRICE_KEY_CANDIDATES,
    ...SUPPLIER_QTY_KEY_CANDIDATES,
    ...SUPPLIER_UNIT_KEY_CANDIDATES,
    "supplier_name",
    "invoice_date",
    "no_prices",
  ])
  for (const k of consumed) skip.add(k)
  let best = ""
  for (const [k, v] of Object.entries(row)) {
    ...
    if (/^col\d+$/i.test(k) || /שם|תיאור|תאור|פריט|מוצר|פירוט|desc|item|name|product/i.test(kl) || /שם|תיאור|תאור|פריט|מוצר|פירוט/.test(k)) {
...
function normalizeSupplierExtractResult(parsed: unknown): ExtractResult {
  const o = parsed && typeof parsed === "object" ? (parsed as Record<string, unknown>) : {}
  ...
  for (const raw of rawItems) {
    ...
    let name =
      pickFirstStringField(row, SUPPLIER_NAME_KEY_CANDIDATES) ||
      findRowStringByNormalizedKeys(row, [
        "שם",
        "שםפריט",
        "שםמוצר",
        "תיאורמוצר",
    ...
    if (!name) {
      name = inferSupplierNameFromLooseKeys(row, consumed)
    }

    const skuRaw =
      pickFirstStringField(row, SUPPLIER_SKU_KEY_CANDIDATES) ||
      findRowStringByNormalizedKeys(row, ["מקט", 'מק"ט', "מק״ט", "ברקוד", "קוד", "קודפריט", "מפתחפריט", "sku", "barcode"])
    const sku = skuRaw || undefined

    let price = pickFirstNumberField(row, SUPPLIER_PRICE_KEY_CANDIDATES)
    if (Number.isNaN(price)) {
      price = findRowNumberByNormalizedKeys(row, [
        "מחיר",
        "מחיר חשבונית",
        "מחיר ליחידה",
        "מחיר יחידה",
    ...
    let qty = pickFirstNumberField(row, SUPPLIER_QTY_KEY_CANDIDATES)
    if (Number.isNaN(qty)) {
      qty = findRowNumberByNormalizedKeys(row, ["כמות", "כמויות", "qty", "quantity"])
    }
    ...
    if (!name.trim()) {
      ...
      else if (price > 0 || qty > 0) name = "פריט ללא שם"
    }
...
function finishExtractResult(type: ExtractType, parsed: unknown): ExtractResult {
  ...
  if (type === "p") return normalizeSupplierExtractResult(parsed)
...
function quickDetectSheetDocType(rows: Record<string, unknown>[]): DetectedDocType | null {
  ...
  const hasDescCol = /תיאור|תאור|שם\s*פריט|תיאור\s*מוצר|(^|\s)שם(\s|$)|description|פירוט|פריט|item\s*name|product/i.test(
    keysStr
  )
