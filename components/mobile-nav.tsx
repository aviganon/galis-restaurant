...
export function MobileNav({ currentPage, setCurrentPage, userRole, isSystemOwner, userPermissions, isImpersonating, onStopImpersonate }: MobileNavProps) {
  ...
  return (
    <>
          ...
          {mainItems(t, userRole, userPermissions, isSystemOwner, isImpersonating).map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => handleClick(item.id)}
                className={cn("flex-1 flex flex-col items-center justify-center gap-1 relative transition-all duration-200", isActive && "text-accent")}>
                {isActive && <span className="absolute top-1 w-5 h-1 rounded-full bg-accent" />}
                <item.icon className={cn("w-5 h-5 transition-transform duration-200", isActive ? "text-primary-foreground scale-110" : "text-primary-foreground/60")} />
                <span className={cn("text-[10px] font-medium transition-colors", isActive ? "text-primary-foreground" : "text-primary-foreground/50")}>
                  {item.label}
                </span>
              </button>
            )
      ...
      {showMore && (
        <>
          <div className="fixed inset-0 z-40 bg-background/60 backdrop-blur-sm md:hidden" onClick={() => setShowMore(false)} />
              ...
              <div className="grid grid-cols-3 gap-3 max-h-[50vh] overflow-y-auto hide-scrollbar">
                {moreItems(t, userRole, userPermissions, isSystemOwner, isImpersonating).map((item) => (
                  <button key={item.id} type="button" onClick={() => handleClick(item.id)}
                    ...
                    <item.icon className={cn("w-6 h-6", currentPage === item.id ? "text-accent" : "text-primary-foreground/70")} />
                    <span className={cn("text-xs font-medium", currentPage === item.id ? "text-primary-foreground" : "text-primary-foreground/70")}>
                      {item.label}
                    </span>
                  </button>
                ))}
